...
class ActionWithMultipleRequestMethodsTest extends Specification implements CompilerTest {
    ...
    def "Should generate separate POST action for controller method"() {
        ...
        when:
            Consumer<ActionBuilder> actionSpec = PingControllerActions.pingPOST(request)
    ...
    def "Should generate separate PUT action for controller method"() {
        ...
        when:
            Consumer<ActionBuilder> actionSpec = PingControllerActions.pingPUT(request)
            def action = new ActionBuilder('ping').with(actionSpec).build()
